...
class WeightBaseList:
    def __init__(self, neuron_dim: List[int], dataType:int=RAND) -> None:
        ...
        else:self.core = np.empty(self.total_size,dtype=np.float32)         # Build structured views
    ...
    def copy(self) -> "WeightBaseList":
        # Returns a deep copy
        new_obj = WeightBaseList(self.neuron_dim, dataType=EMPTY)
